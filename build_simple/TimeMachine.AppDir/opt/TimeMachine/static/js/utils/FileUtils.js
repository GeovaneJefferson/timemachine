...
export class FileUtils {
    ...
    static getIconForFile(filename) {
    ...
    static getColorForFile(filename) {
        const ext = filename.split('.').pop().toLowerCase();
        const colors = {
            'blend': 'text-amber-600',
            'blend1': 'text-amber-600',
            'pdf': 'text-red-500',
            'doc': 'text-blue-500',
            'docx': 'text-blue-500',
    ...
    static formatRelativeDate(dateString) {
        ...
        return `${formattedMonth}-${formattedDay}-${yearShort}, ${time24H}`;
    }
    ...
    static getFileIconDetails(filename) {
