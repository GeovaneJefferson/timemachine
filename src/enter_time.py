...
class ENTERTIMEMACHINE(QWidget):
    ...
    def start_restore(self, getDate, getTime):
        ...
        try:
            ...
            for _ in self.filesToRestore:
                sub.run(
                    f"{COPY_RSYNC_CMD} {MAININIFILE.ini_external_location()}/{BASE_FOLDER_NAME}/"
                    f"{BACKUP_FOLDER_NAME}/{getDate}/{getTime}/{self.current_folder}/"
                    f"{self.filesToRestore[count]} {HOME_USER}/{self.current_folder}/",
                    shell=True)
    ...
    async def start(self):
        ...
        if is_connected(MAININIFILE.ini_hd_name()):
