...
class Daemon:
    ...
    async def scan_and_backup(self):
        ...
        self.ignored_folders = set(os.path.abspath(p) for p in server.load_ignored_folders_from_config())
        exclude_hidden_master_switch = server.get_database_value(
            section='EXCLUDE', 
    ...
    async def run(self):
        ...
        while not self.should_exit:
            ...
            if not os.path.exists(server.SOCKET_PATH):
                logging.warning(f"Daemon PID file {server.SOCKET_PATH} was removed externally. Shutting down.")
            ...
            while elapsed < total_wait and not self.should_exit:
                try:
                    ...
                    # Socket file was removed
                    if not os.path.exists(server.SOCKET_PATH):
                        logging.warning(f"Socket file {server.SOCKET_PATH} was removed externally. Shutting down.")
                        self.signal_handler(signal.SIGTERM, None) # This sets self.should_exit = True
                        break
