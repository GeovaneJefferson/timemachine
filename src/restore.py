    ...
    def start_restore(self, getDate, getTime):
        ...
        try:
            ...
            for _ in self.filesToRestore:
                sub.run(
                    f"{copyCmd} {self.getExternalLocation}/{folderName}/{getDate}/{getTime}/{self.requested}/{self.filesToRestore[count]} {homeUser}/{self.requested}/ &",
                    shell=True)
                count += 1
            ...
            for _ in self.filesToRestoreWithSpace:
                sub.run(
                    f'{copyCmd} {self.getExternalLocation}/{folderName}/{getDate}/{getTime}/{self.requested}/"{self.filesToRestoreWithSpace[count]}" {homeUser}/{self.requested}/ &',
                    shell=True)
                count += 1
