...
def search_in_all_date_time_file(
    ...
    for i in range(len(all_dates_list)):
        ...
        for time in reversed(os.listdir(date_path)):  # Start from the latest time folder
            time_path = os.path.join(date_path + '/' + time)
            
            # Add all time to the list
    ...
    #         # Loop through each time folder in the current date folder
    #         for time_folder in reversed(os.listdir(date_path)):
