...
class PreBackup(QWidget):
    def __init__(self):
        ...
        if os.path.exists(MAININIFILE.exclude_applications_location()):
            sub.run(f"rm -rf {MAININIFILE.exclude_applications_location()}",shell=True)
    ...
    def flatpak_data_option(self):
        ...
        try:
            ...
            self.flatpakDataSize=os.popen(f"du -hs {MAININIFILE.ini_external_location()}/{BASE_FOLDER_NAME}/{APPLICATIONS_FOLDER_NAME}")
            self.flatpakDataSize=self.flatpakDataSize.read().strip("\t").strip("\n").replace(f"{MAININIFILE.ini_external_location()}/{BASE_FOLDER_NAME}/{APPLICATIONS_FOLDER_NAME}", "").replace("\t", "")
    ...
    def on_application_clicked(self):
        ...
        if self.application_packages_Checkbox.isChecked():
            restore_applications_packages=True
            ...
            restore_applications_packages=False
    ...
    def on_flatpak_clicked(self):
        ...
        if self.flatpakCheckBox.isChecked():
            ...
            restore_flatpaks_programs=True
            ...
            restore_flatpaks_programs=False
    ...
    def on_continue_button_clicked(self):
        if self.application_packages_Checkbox.isChecked() == True:
            ...
            if not os.path.exists(MAININIFILE.exclude_applications_location()):
                    sub.run(f"{CREATE_CMD_FILE} {MAININIFILE.exclude_applications_location()}", shell=True)
            ...
            if self.exclude_app_list:
                ...
                CONFIG=configparser.ConfigParser()
                CONFIG.read(SRC_USER_CONFIG)
                with open(MAININIFILE.exclude_applications_location(), 'w') as configfile:
